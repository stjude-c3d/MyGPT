...
def fix_correctness_scores(models, embeds):
    for model in models:
...
def fix_parameter_correctness_scores():
    for chunksize in ['500', '1000', '1500']:
        for suffix in ['-overlap', '']:
            correctness_data = pd.read_csv(f'evaluation/scores/parameter_scores/answer_correctness_old/correctness-{chunksize}{suffix}.csv')
            similarity_data = pd.read_csv(f'evaluation/scores/parameter_scores/answer_relevancy_similarity/evaluation-{chunksize}{suffix}.csv')
            ...
            data = [{"question": q, "answer": a, "contexts": c, "ground_truth": g, "answer_correctness": correctness}
                    for q, a, c, g, correctness in zip(questions, answers, contexts, ground_truths, answer_correctness)]
            df = pd.DataFrame.from_records(data=data)
            df.to_csv(f'evaluation/scores/parameter_scores/answer_correctness/correctness-{chunksize}{suffix}.csv', index=False)

fix_parameter_correctness_scores()
