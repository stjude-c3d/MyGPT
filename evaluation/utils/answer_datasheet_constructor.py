...
def reconstruct_speed_data(models, loc):
    data = []
    for model in models:
        df = pd.read_json(f'evaluation/answer_speeds/{loc}/{model}-speeds.json')
    ...
    final_df.to_csv(f'evaluation/documents/{loc}_speeds.csv')
...
models = ['gemma', 'gemma2', 'llama2', 'llama3', 'mistral', 'vicuna']
reconstruct_speed_data(models, 'local')