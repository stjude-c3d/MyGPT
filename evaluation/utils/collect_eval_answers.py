...
old_embeds = ['all-MiniLM-L6-v2', 'all-MiniLM-L12-v2', 'multi-qa-MiniLM-l6-cos-v1', 'all-mpnet-base-v2', 'multi-qa-mpnet-base-dot-v1', 'paraphrase-albert-small-v2']
...
for embed, shorthand in zip(embeds, embed_shorthands):
    ...
    for model in models:
        for j, (question, library, groundtruth) in enumerate(zip(question_list, lib_list, groundtruth_list)):
            if f'mygpt-{library}' in datasets:
                # Log query info
                print('\n')
                print(f'MODEL: {model}; EMBEDDING: {embed};')
                print(f'Loading Question {str(j+1)}...')

                dataset_name = f'mygpt-{library}-{shorthand}'
                contexts = pd.DataFrame.from_records(f'evaluation/eval_context/{shorthand}/{dataset_name}.json')['context'].tolist()[j]
